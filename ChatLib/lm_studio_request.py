...
# 添加配置变量
LM_STUDIO_API_URL = "http://127.0.0.1:11435/"

class LMStudioLLMRequest:
    ...
    def get_response(self, data, stop_strings=None, webui=False):
        response = requests.post(self.api_url + "v1/chat/completions", json=data, stream=True)  # 使用流式输出
        result = ""
        with open("output.log", "a", encoding="utf-8") as f:
            for line in response.iter_lines():
                if line:
                    ...
                    response_content = json_data.get("choices", "")[0].get("delta", "").get("content", "")
    ...
    def start_chat(self):
        ...
        return True
def main():
    llm_request = LMStudioLLMRequest(LM_STUDIO_API_URL)
