...
set TMP=%CD%\NicoChatRunTime
set TEMP=%CD%\NicoChatRunTime

(call conda deactivate && call conda deactivate && call conda deactivate) 2>nul

set INSTALL_DIR=%cd%\NicoChatRunTime
set CONDA_ROOT_PREFIX=%cd%\NicoChatRunTime\conda
set INSTALL_ENV_DIR=%cd%\NicoChatRunTime\env
set PIP_CMD=%cd%\NicoChatRunTime\env\python -m pip
set PYTHON_CMD=%cd%\NicoChatRunTime\env\python
...
call %PIP_CMD% install -r "%cd%\requirements.txt"